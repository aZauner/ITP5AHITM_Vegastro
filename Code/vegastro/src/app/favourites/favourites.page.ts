          ...
          if (response.data.status != 404) {
            if (response.data.favouriteRestaurants[0] != undefined) {
              ...
              for (const restaurant of response.data.favouriteRestaurants) {
                ...
                service.addDynamicComponentFav({ id: restaurant._id, image: restaurant.image.id ? restaurant.image.id : null, restaurantName: restaurant.restaurantName, description: desc, type: restaurant.type, stars: restaurant.stars, menu: restaurant.menu, isFav: true, fromFavPage: true })
              }
            } else {
